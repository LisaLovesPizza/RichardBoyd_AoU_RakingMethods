...
class EmpiricalCDF():
    ...
    # The inverse of the cumulative distribution function, which is also
    # known as the quantile function or the percent-point function.
    # This should return the following, for a probability p as input:
    #
    #     inv(p) = inf{x | p <= ECDF(x)}
    #
    # Since the support points for discrete distributions are defined on
    # the integers, this is equivalent to the minimum value of x for
    # which p <= ECDF(x) is true.
    #
    # To quote Wikipedia: "the quantile function returns a threshold
    # value x below which random draws from the CDF would fall p percent
    # of the time."
    def inv(self, p):
        """
        ...
        if index < 0 or index > len(self.x): #self.n:
            print('*** BAD INDEX: {0}'.format(index))
            print('\t     p: {0}'.format(p))
            print('\tself.n: {0}'.format(self.n))
            print('\tself.x: {0}'.format(self.x))
            print('\tself.y: {0}'.format(self.y))
        assert index >= 0 and index <= len(self.x) #self.n
        if index < len(self.x):
            return self.x[index]
        else:
            return self.x[-1]
